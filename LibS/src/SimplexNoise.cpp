...
T SimplexNoise<T>::rawNoise2(const T x, const T y) const
{
    ...
    T F2 = 0.36602540378443864676372317075294; //0.5 * (sqrt(3.0) - 1.0);
    // Hairy factor for 2D
    T s = (x + y) * F2;
    int i = fastFloor( x + s );
    int j = fastFloor( y + s );

    T G2 = 0.21132486540518711774542560974902; //(3.0 - sqrt(3.0)) / 6.0;
    T t = (i + j) * G2;
...
T SimplexNoise<T>::rawNoise3(const T x, const T y, const T z) const
{
    ...
    T G3 = 1.0 / 6.0; // Very nice and simple unskew factor, too
    T t = (i + j + k) * G3;
...
T SimplexNoise<T>::rawNoise4(const T x, const T y, const T z, const T w) const
{
    ...
    T F4 = 0.30901699437494742410229341718282; //(sqrt(5.0) - 1.0) / 4.0;
    T G4 = 0.13819660112501051517954131656344; //(5.0 - sqrt(5.0)) / 20.0;
...
{
    return octaveNoise1(x, octaves, persistence, scale) * (higherBound - lowerBound) * 0.5 + (higherBound + lowerBound) * 0.5;
}
template <class T>
...
{
    return octaveNoise2(x, y, octaves, persistence, scale) * (higherBound - lowerBound) * 0.5 + (higherBound + lowerBound) * 0.5;
}
template <class T>
...
{
    return octaveNoise3(x, y, z, octaves, persistence, scale) * (higherBound - lowerBound) * 0.5 + (higherBound + lowerBound) * 0.5;
}
template <class T>
...
{
    return octaveNoise4(x, y, z, w, octaves, persistence, scale) * (higherBound - lowerBound) * 0.5 + (higherBound + lowerBound) * 0.5;
}
